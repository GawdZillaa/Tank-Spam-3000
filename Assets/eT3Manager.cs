...
public class eT3Manager : MonoBehaviour {
    ...
    void Start()
    {
        ...
        GameOver.GetComponent<GameOverManager>().AiRegisterArmy(3);
    ...
    void Update()
    {
        ...
        if (isAuto == false)
        {
            if ((Vector3.Distance(transform.position, new Vector3(TargetFrontline.x, transform.position.y, TargetFrontline.z) ) > .1f) && killSwitch == false && CommandRecieved == true
             && shouldMove == true)
            {
                if (hasFlipped == false)
                {
    ...
    //            Debug.Log("Manual Move...");
                ...
                transform.position = Vector3.MoveTowards(transform.position, new Vector3(TargetFrontline.x, transform.position.y, TargetFrontline.z), moveTotalSpeed);
    ...
    public void takeDamage(int amount)
    {
        ...
        if (health <= 0 || health == 0)
        {
            ...
            if (stopHealth == false)
            {
                ...
                GameOver.GetComponent<GameOverManager>().PlayerRegisterKill(3);

                audio.PlayOneShot(explode);
                resources.GetComponent<ResourceManagerG>().killResource(45);
